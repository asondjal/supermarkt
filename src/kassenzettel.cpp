...
std::string Kassenzettel::CreateKassenzettel() const {
  ...
  oss << std::left << std::setw(20) << "Produkt" << std::setw(15) << "Menge " << std::setw(20)
      << "Preis [€]" << "\n";
  ...
  for (const auto& produkt : warenkorb_.GetProdukte()) {
    oss << std::fixed << std::setprecision(2) << std::left << std::setw(20) << produkt->GetName()
        << std::setw(15) << produkt->GetMenge() << std::setw(20) << produkt->GetPreis() << "\n";
  }

  oss << std::string(50, '-') << "\n";
  oss << std::left << std::setw(35) << "Gesamtpreis:" << std::fixed << std::setprecision(2)
