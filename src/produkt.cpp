 ...
 */
Datum Produkt::GetHaltbarkeit() const { return haltbarkeit_; }

/**
 * @brief Ausgabe von der Abteilung eines Produktes
 * @return Abteilung eines Produktes
 */
std::string Produkt::GetAbteilung() const { return abteilung_; }
...
void Produkt::Display() const {
  std::cout << "Name: " << name_ << "\n"
            << "Menge: " << menge_ << " kg\n"
            << "Preis: " << preis_ << " €\n"
            << "Ablaufdatum: " << haltbarkeit_.GetTag() << "." << haltbarkeit_.GetMonat() << "."
            << haltbarkeit_.GetJahr() << "\n"
