...
std::string Supermarkt::CreateProduktDatabase() const {
  ...
  oss << std::left << std::setw(20) << "Produkt" << std::setw(15) << "ID" << std::setw(20)
      << "Preis [€]" << std::setw(15) << "Menge" << "\n";
  oss << std::string(70, '-') << "\n";

  for (const auto& produkt : produkte_) {
    oss << std::fixed << std::setprecision(2) << std::left << std::setw(20) << produkt->GetName()
        << std::setw(15) << produkt->GetID() << std::setw(20) << produkt->GetPreis()
        << std::setw(15) << produkt->GetMenge() << "\n";
  }
  oss << std::string(70, '-') << "\n";
  oss << std::left << std::setw(35) << "Umsatz: " << std::fixed << std::setprecision(2)
 ...
 */
void Supermarkt::AddKunde(std::shared_ptr<Kunde> kunde) { kunden_.insert(kunde); }

/**
 * @brief Entfernen eines Kunden
 */
void Supermarkt::RemoveKunde(std::shared_ptr<Kunde> kunde) { kunden_.erase(kunde); }
...
std::string Supermarkt::CreateKundeDatabase() const {
  ...
  std::ostringstream oss;
  oss << std::left << std::setw(20) << "Kunde" << std::setw(15) << "ID" << std::setw(20) << "Alter"
      << std::setw(15) << "Geschlecht" << "\n";
  oss << std::string(70, '-') << "\n";

  for (const auto& kunde : kunden_) {
    oss << std::left << std::setw(20) << kunde->GetName() << std::setw(15) << kunde->GetKundeID()
        << std::setw(20) << kunde->GetAge() << std::setw(15) << kunde->GetGender() << "\n";
  }
  oss << std::string(70, '-') << "\n";
  oss << std::left << std::setw(35) << "Anzahl Kunden: " << std::setw(14) << kunden_.size() << "\n";
  oss << "===========================================================================\n";
 ...
 */
void Supermarkt::AddWarenkorb(Warenkorb& warenkorb) { warenkoerbe_.insert(&warenkorb); }

/**
 * @brief Entfernen eines Warenkorbes
 */
void Supermarkt::RemoveWarenkorb(Warenkorb& warenkorb) { warenkoerbe_.erase(&warenkorb); }
...
std::string Supermarkt::CreateWarenkorbDatabase() const {
  ...
  std::filesystem::create_directories("./data/supermarkt_" + std::to_string(supermarkt_ID_));
  std::string warenkorbdatabase_path =
      "./data/supermarkt_" + std::to_string(supermarkt_ID_) + "/warenkoerbe.txt";
  ...
  file << "Adresse: " << adresse_ << "\n\n";
  std::ostringstream oss;
  oss << std::left << std::setw(20) << "Kunde" << std::setw(15) << "ID" << std::setw(20)
      << "Gesamtpreis [€]" << "\n";
  oss << std::string(70, '-') << "\n";

  for (const auto& warenkorb : warenkoerbe_) {
    oss << std::fixed << std::setprecision(2) << std::left << std::setw(20)
        << warenkorb->GetKunde()->GetName() << std::setw(15) << warenkorb->GetKunde()->GetKundeID()
 ...
 */
void Supermarkt::AddHaendler(Haendler& haendler) { haendler_.insert(&haendler); }

/**
 * @brief Entfernen eines Händlers aus dem System
 */
void Supermarkt::RemoveHaendler(Haendler& haendler) { haendler_.erase(&haendler); }
...
std::string Supermarkt::CreateHaendlerDatabase() const {
  ...
  file << "Adresse: " << adresse_ << "\n\n";
  std::ostringstream oss;
  oss << std::left << std::setw(20) << "Haendler" << std::setw(15) << "ID" << std::setw(20)
      << "Alter" << std::setw(15) << "Geschlecht" << "\n";
  oss << std::string(70, '-') << "\n";

  for (const auto& haendler : haendler_) {
    oss << std::left << std::setw(20) << haendler->GetName() << std::setw(15)
        << haendler->GetHaendlerID() << std::setw(20) << haendler->GetAge() << std::setw(15)
        << haendler->GetGender() << "\n";
  }

  oss << std::string(70, '-') << "\n";
  oss << std::left << std::setw(35) << "Anzahl Haendler: " << std::setw(14) << haendler_.size()
      << "\n";
  oss << "===========================================================================\n";
