...
class Kassenzettel {
 private:
  ...
  std::shared_ptr<Konto>
      konto_;  // Nicht konstant, da Auszahlungen und Einzahlungen möglich bleiben müssen
  std::string filename_;
  mutable std::mutex kassenzettelMutex_;

 public:
  Kassenzettel(const Datum& datum, std::shared_ptr<Kunde> kunde, const Haendler& haendler,
               const Warenkorb& warenkorb, std::shared_ptr<Konto> konto);
